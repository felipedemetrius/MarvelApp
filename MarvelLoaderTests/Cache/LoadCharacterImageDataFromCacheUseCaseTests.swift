//
//  LoadCharacterImageDataFromCacheUseCaseTests.swift
//  MarvelLoaderTests
//
...
class LoadCharacterDataFromCacheUseCaseTests: XCTestCase {
    ...
    private func makeSUT(currentDate: @escaping () -> Date = Date.init, file: StaticString = #filePath, line: UInt = #line) -> (sut: LocalCharacterImageDataLoader, store: CharacterImageDataStoreSpy) {
    ...
    private func expect(_ sut: LocalCharacterImageDataLoader, toCompleteWith expectedResult: Result<Data, Error>, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        action()
