//
//  CharacterCacheIntegrationTests.swift
//  MarvelLoaderTests
//
...
class CharacterCacheIntegrationTests: XCTestCase {
    ...
    func test_loadImageData_deliversSavedDataOnASeparateInstance() throws {
        ...
        let url = URL(string: image.urlImage)!
    ...
    func test_saveImageData_overridesSavedImageDataOnASeparateInstance() throws {
        ...
        save(firstImageData, for: url, with: imageLoaderToPerformFirstSave)
        save(lastImageData, for: url, with: imageLoaderToPerformLastSave)
    ...
    private func makeFeedLoader(currentDate: Date = Date(), file: StaticString = #filePath, line: UInt = #line) throws -> LocalCharacterLoader {
        let storeURL = testSpecificStoreURL()
        let store = try CoreDataCharacterStore(storeURL: storeURL, contextQueue: .main)
    ...
    private func makeImageLoader(file: StaticString = #filePath, line: UInt = #line) throws -> LocalCharacterImageDataLoader {
        let storeURL = testSpecificStoreURL()
        let store = try CoreDataCharacterStore(storeURL: storeURL, contextQueue: .main)
    ...
    private func save(_ feed: [Character], with loader: LocalCharacterLoader, file: StaticString = #filePath, line: UInt = #line) {
        do {
    ...
    private func validateCache(with loader: LocalCharacterLoader, file: StaticString = #filePath, line: UInt = #line) {
        do {
    ...
    private func expect(_ sut: LocalCharacterLoader, toLoad expectedFeed: [Character], file: StaticString = #filePath, line: UInt = #line) {
        do {
    ...
    private func save(_ data: Data, for url: URL, with loader: LocalCharacterImageDataLoader, file: StaticString = #filePath, line: UInt = #line) {
        do {
    ...
    private func expect(_ sut: LocalCharacterImageDataLoader, toLoad expectedData: Data, for url: URL, file: StaticString = #filePath, line: UInt = #line) {
        do {
